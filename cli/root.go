// Package cli builds the crossref command tree on top of the crossref library.
package cli
...
func Root() *cobra.Command {
	app := &App{cfg: crossref.DefaultConfig()}

	root := &cobra.Command{
		Use:   "crossref",
		Short: "Browse Crossref scholarly metadata by DOI",
		Long: `crossref reads the Crossref REST API (https://api.crossref.org), which
provides open DOI metadata for scholarly works. No API key is required.
The polite pool (higher rate limits) is used automatically by including
a mailto: address in the User-Agent header.

crossref is an independent tool and is not affiliated with Crossref.`,
		SilenceUsage:  true,
		SilenceErrors: true,
...
func (a *App) setup() error {
	...
	a.client = crossref.NewClient(a.cfg)
	return nil
}
